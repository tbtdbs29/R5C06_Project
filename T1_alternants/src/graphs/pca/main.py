...
def biplot_variables(pca: PCA, feature_names: list[str], top_k: int = 30) -> pd.DataFrame:
    ...
    dim1_var = pca.explained_variance_ratio_[0] * 100
    dim2_var = pca.explained_variance_ratio_[1] * 100
    plt.xlabel(f"Dimension 1 ({dim1_var:.2f}%)", fontweight='bold')
    plt.ylabel(f"Dimension 2 ({dim2_var:.2f}%)", fontweight='bold')

    plt.title("Correlation circle (PCA)",fontsize=20, fontweight='bold')
    plt.xlim(-1.1, 1.1)
    plt.ylim(-1.1, 1.1)
